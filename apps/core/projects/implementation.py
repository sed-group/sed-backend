...
def impl_post_project(project: models.ProjectPost) -> models.Project:
    with get_connection() as con:
        res = storage.db_post_project(con, project)
        con.commit()
        return res
...
def impl_delete_subproject(project_id: int, subproject_id: int) -> bool:
    try:
        with get_connection() as con:
            storage.db_delete_subproject(con, project_id, subproject_id)
            con.commit()
    except exc.SubProjectNotFoundException:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
